...
namespace ServiceCustomerWithPolly
{
    public static class PolicyBuilder
    {
        public static ISyncPolicy CreatePolly()
        {
            ...
            // 如果重试2次中只要成功1次，就直接返回调用结果
            // 如果重试2次都失败，第3次再次失败，就会发生故障
            // 重试失败之后是断路器策略，所以这个故障会被断路器接收，当断路器收到2次故障，就会触发熔断，也就是说断路器开启
            // 断路器开启的5秒内，任何故障或者操作，都会通过断路器到达回退策略，触发降级操作
            // 5秒后，断路器进入到半开启状态，操作可以正常执行
